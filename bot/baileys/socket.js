...
export const obterTodosGrupos = async (c) => {
    ...
    for (let [key, value] of Object.entries(grupos)) {
        gruposInfo.push({
            id: key,
            nome: value.subject,
            participantes: value.participants.length,
        });
    }
    return gruposInfo;
};

export const adicionarMembroGrupo = async (c, id_grupo, numeroMembro) => {
    try {
        await c.groupAdd(id_grupo, [numeroMembro]);
        return true;
    } catch (err) {
...
export const removerMembroGrupo = async (c, id_grupo, numeroMembro) => {
    try {
        await c.groupRemove(id_grupo, [numeroMembro]);
        return true;
    } catch (err) {
...
export const alterarNomeGrupo = async (c, id_grupo, novoNome) => {
    try {
        await c.groupUpdateSubject(id_grupo, novoNome);
        return true;
    } catch (err) {
...
export const alterarDescricaoGrupo = async (c, id_grupo, novaDescricao) => {
    try {
        await c.groupUpdateDescription(id_grupo, novaDescricao);
        return true;
    } catch (err) {
        console.error('Erro ao alterar descrição do grupo:', err);
        return false;
    }
};
